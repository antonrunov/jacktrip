...
JTMainWindow::JTMainWindow()
{
  ...
  // RtAudio
  QCheckBox* chk_rtaudio = new QCheckBox("Use RtAudio", this);
  chk_rtaudio->setChecked(checkConfValue(conf, USE_RT_AUDIO, 0).toBool());
  ...
  setGridRowVisibility(conf_layout, row, chk_advanced->isChecked() && (!chk_rtaudio->isChecked()));
  ++row;
  ...
  connect(chk_advanced, &QCheckBox::toggled, this, [=] () {
      ...
      setGridRowVisibility(conf_layout, row_adv+1, chk_advanced->isChecked());
      setGridRowVisibility(conf_layout, row_adv+2, chk_advanced->isChecked() && (!chk_rtaudio->isChecked()));
      setGridRowVisibility(conf_layout, row_adv+3, chk_advanced->isChecked() && (!chk_rtaudio->isChecked()));
      });
